...
패키지 데이터 → 레이아웃 계산 → 순차 스폰 → 물리 시뮬레이션 → 정착 감지 후 일시정지

```
ResultView
└── JengaScene
    ├── Physics (paused when settled)
...
- **통합 테스트**: 스폰 시퀀스가 중복 실행되지 않는지, 모든 블록 정착 후 완료 상태 콜백 호출
- **수동 테스트**:
  - 결과 화면 진입 시 블록이 위에서 순차적으로 떨어짐
  - 일정 시간 후 블록이 안정적으로 정착
  - 패널 호버/블록 호버가 정상 동작
