...
ResultView에서 패널 토글 상태를 관리하고, 모바일은 기본 접힘/데스크톱은 기본 펼침으로 초기화한다. 패널 영역은 `isPanelOpen` 상태에 따라 높이/투명도를 전환하며, 데스크톱에서도 접기/펼치기를 허용한다. 데스크톱에서 패널이 접히면 캔버스 오른쪽 패딩을 제거해 시야를 확장한다.
